  ...
  return (
    ...
    >
      <div className="container relative mx-auto max-w-6xl px-4 sm:px-6">
        <nav
          className={`relative flex items-center justify-between overflow-hidden rounded-[16px] sm:rounded-[20px] border px-5 py-2.5 sm:px-7 sm:py-3 transition-all duration-500 ${
            scrolled
          ...
          <div className="hidden items-center gap-1 min-[901px]:flex">
            {LINKS.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className={`relative px-4 py-2 text-label font-semibold uppercase transition-colors duration-200 rounded-lg hover:bg-white/10 ${
                  scrolled
            ...
            <a
              href="#contact"
              className="ml-3 rounded-xl bg-black px-6 py-2.5 text-label font-bold text-white uppercase shadow-[0_8px_24px_rgba(0,0,0,0.25)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-neutral-900"
            >
              Book A Consultation
            </a>
          </div>
        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div
              ...
              className="absolute inset-x-4 top-full mt-3 flex flex-col gap-1 rounded-[20px] border border-primary/10 bg-white/98 backdrop-blur-2xl p-6 shadow-[0_30px_80px_rgba(3,46,151,0.18)] min-[901px]:hidden"
            >
              {/* Logo in mobile menu */}
              ...
              {LINKS.map((link) => (
                <a
                  ...
                  className="flex items-center justify-between px-2 py-3 text-h4 font-semibold uppercase text-ink hover:text-primary rounded-xl hover:bg-bg-soft transition-all duration-200"
                >
                  {link.label}
              ...
              <a
                href="#contact"
                className="mt-4 flex w-full items-center justify-center rounded-xl bg-black py-3 text-center text-label font-bold text-white uppercase shadow-md hover:bg-neutral-900"
                onClick={() => setMenuOpen(false)}
              >
                Book A Consultation
              </a>
